...
impl FromStr for Address<NetworkUnchecked> {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let bech32_network = match find_bech32_prefix(s) {
            ...
            prefix => {
              if get_config().bech32_prefix.unwrap() == prefix {
        ...
        let (network, payload) = match data[0] {
            ...
            x => {
              if x == get_config().scripthash_prefix {
                return (Network::Regtest, Payload::ScriptHash(ScriptHash::from_slice(&data[1..]).unwrap()))
              }
              if x == get_config().pubkeyhash_prefix {
                return (Network::Regtest, Payload::PubkeyHash(PubkeyHash::from_slice(&data[1..]).unwrap()))
              }
              return Err(ParseError::Base58(base58::Error::InvalidAddressVersion(x))),
            }
        };
